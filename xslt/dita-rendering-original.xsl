 ...
 "'ABCDEFGHIJKLMNOPQRSTUVWXYZ'"/>
  ...
  "/>
	...
	<xsl:template match="menucascade">
		<xsl:element name="span">
			<xsl:call-template name="commonattributes">
				<xsl:with-param name="default-output-class" select="'menucascade'"/>
			</xsl:call-template>
	...
	<xsl:template match="uicontrol">
		<xsl:element name="span">
			<xsl:call-template name="commonattributes">
				<xsl:with-param name="default-output-class" select="'ph uicontrol'"/>
			</xsl:call-template>
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="codeblock">
		<xsl:if test="node()[string-length() != 0]">
			<div class="code-wrapper">
				<div class="code-bar">
					<div class="row">
						<div class="col-10 col-sm-11">
							<xsl:value-of select="parent::fig/title"/>
						</div>
				...
				<div class="onthispage-content-code">
					<pre>
						<code class="codeblock {@outputclass}">
							<xsl:apply-templates select="node()" />
						</code>
					</pre>
				</div>
	...
	<xsl:template match="note">
		...
		<xsl:param name="title">
			<xsl:choose>
				...
				<xsl:when test="normalize-space($type) = ''">Note</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="concat(translate(substring($type,1,1), $vLower, $vUpper), substring($type, 2), substring(' ', 1 div not(position()=last())))"/>
				</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:variable name="color">
			<xsl:call-template name="getNoteColor">
				<xsl:with-param name="type" select="@type"/>
				<xsl:with-param name="otherType" select="@othertype"/>
			</xsl:call-template>
		</xsl:variable>												   
		<xsl:element name="div">
			<xsl:call-template name="commonattributes">
				<xsl:with-param name="default-output-class" select="concat('note-wrap ', $color, ' note_', $color, ' ', $outputclass)"/>
			</xsl:call-template>
			<div class="notice-inner">
				<div class="heading">
					<span class="icon {$color}-text">
